...
import { localDate, localNoon, localTime, planFor } from "../sun.ts";
...
export function planCommand(date: string | undefined, opts: PlanOpts): void {
  ...
  console.log(`${cfg.location.lat}, ${cfg.location.lon} (${tz}) — sunrise ${cfg.schedule.sunriseOffsetMin >= 0 ? "+" : ""}${cfg.schedule.sunriseOffsetMin} min; daemon starts ${pad(hour)}:${pad(minute)}\n`);
  console.log("date        sunrise  shoot at");
  let late = 0;
  for (let i = 0; i < days; i++) {
    const noon = localNoon(start, tz);
    const d = localDate(new Date(noon.getTime() + i * 86_400_000), tz);
    const p = planFor(d, cfg.location, cfg.schedule.sunriseOffsetMin);
    const daemon = new Date(localNoon(d, tz).getTime() + ((hour - 12) * 60 + minute) * 60_000);
    const tooLate = p.fireAt < daemon;
    if (tooLate) late++;
    console.log(`${d}  ${localTime(p.sunrise, tz)}    ${localTime(p.fireAt, tz)}${tooLate ? "   ⚠ before daemon_start" : ""}`);
  }
